...
void sm3_compress_blocks_avx2_test(
        uint32_t digest[8],
        const uint8_t *data)
{
    ...
    __m256i W[68];
    A=_mm256_set1_epi32(digest[0]);
    B=_mm256_set1_epi32(digest[1]);
    C=_mm256_set1_epi32(digest[2]);
    D=_mm256_set1_epi32(digest[3]);
    E=_mm256_set1_epi32(digest[4]);
    F=_mm256_set1_epi32(digest[5]);
    G=_mm256_set1_epi32(digest[6]);
    H=_mm256_set1_epi32(digest[7]);
    for(int i=0;i<16;i++)
{
    W[i]=_mm256_loadu_si256(
        (__m256i *)(data+i*32)
    );
}
    for(int i=16;i<68;i++)
